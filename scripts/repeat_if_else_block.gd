...
## Visual widget for a REPEAT-IF-ELSE blueprint block.
## Renders as a Scratch-like orange nested block.
## Slots that are null in the blueprint become dropdowns (OptionButton).
## Slots with a fixed value become read-only labels.
##
## Emits changed() whenever any dropdown value changes.

signal changed

# ── Slot data (set by _setup before adding to tree) ───────────────────────────
# Use null for a blank dropdown slot, or a string for a fixed pre-filled slot.
var repeat_count    : int    = 6        # always fixed, shown as label
...
const ACT_OPTIONS  : Array[String] = ["move_up", "move_down", "move_left", "move_right"]
const ACT_SYMBOLS  : Array[String] = ["↑ MOVE UP", "↓ MOVE DOWN", "← MOVE LEFT", "→ MOVE RIGHT"]

const COLOR_OUTER  : Color = Color(0.95, 0.55, 0.05, 1.0)   # orange shell
const COLOR_INNER  : Color = Color(0.80, 0.42, 0.02, 1.0)   # darker inner
...
func _build_ui() -> void:
	...
	count_lbl.text = "  (%d×)" % repeat_count
	count_lbl.add_theme_color_override("font_color", Color(1.0, 1.0, 0.5))
	count_lbl.add_theme_font_size_override("font_size", 15)
	repeat_row.add_child(count_lbl)

	# ── Inner panel (IF body) ─────────────────────────────────────────────────
	var inner_panel := PanelContainer.new()
	var inner_style := StyleBoxFlat.new()
	inner_style.bg_color                   = COLOR_INNER
	inner_style.corner_radius_top_left     = 5
	inner_style.corner_radius_top_right    = 5
	inner_style.corner_radius_bottom_left  = 5
	inner_style.corner_radius_bottom_right = 5
	...
	if then_action == null:
		_then_opt = _make_dropdown(ACT_SYMBOLS)
		then_row.add_child(_then_opt)
		_then_opt.item_selected.connect(func(_i): changed.emit())
	else:
		then_row.add_child(_make_fixed_label(_action_to_symbol(str(then_action))))
	...
	if else_action == null:
		_else_opt = _make_dropdown(ACT_SYMBOLS)
		else_row.add_child(_else_opt)
		_else_opt.item_selected.connect(func(_i): changed.emit())
	else:
		else_row.add_child(_make_fixed_label(_action_to_symbol(str(else_action))))

# ── Helpers ────────────────────────────────────────────────────────────────────
func _make_dropdown(options: Array[String]) -> OptionButton:
	var ob := OptionButton.new()
	ob.custom_minimum_size = Vector2(110, 28)
...
func _action_to_symbol(act: String) -> String:
	match act:
...
func get_then_action() -> CommandBlock.CommandType:
	var act_str : String
	if then_action != null:
		act_str = str(then_action)
	elif _then_opt:
		act_str = ACT_OPTIONS[_then_opt.selected]
	else:
		act_str = "move_up"
	return _str_to_cmd(act_str)

func get_else_action() -> CommandBlock.CommandType:
	var act_str : String
	if else_action != null:
		act_str = str(else_action)
	elif _else_opt:
		act_str = ACT_OPTIONS[_else_opt.selected]
	else:
		act_str = "move_up"
	return _str_to_cmd(act_str)
...
## Returns a CommandBlock configured with the current dropdown values.
## Call this just before executing to get a fresh snapshot.
func build_command_block() -> CommandBlock:
	var cb := CommandBlock.new(CommandBlock.CommandType.REPEAT_IF_ELSE)
	cb.repeat_count    = repeat_count
